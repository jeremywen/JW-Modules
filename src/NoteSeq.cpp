...
struct NoteSeqDisplay : LightWidget {
	...
	void drawLayer(const DrawArgs &args, int layer) override {
		...
		if(layer == 1){
			...
			int rootNote = module == NULL ? 0 : module->params[NoteSeq::NOTE_KNOB_PARAM].getValue();
			for(int i=0;i<ROWS;i++){
...
void NoteSeqWidget::appendContextMenu(Menu *menu) {
	...
	struct NSRepeat16StepsItem : MenuItem {
		NoteSeq *module;
		void onAction(const event::Action &e) override {
			module->repeatFirst16StepsAcross();
		}
	};
	NSRepeat16StepsItem *repeat16Item = new NSRepeat16StepsItem;
	repeat16Item->module = noteSeq;
	...
	GatePulseMsSlider* gateSlider = new GatePulseMsSlider();
	{
		...
		qp->getSeconds = [noteSeq](){ return noteSeq->gatePulseLenSec; };
		qp->setSeconds = [noteSeq](float v){ noteSeq->gatePulseLenSec = v; };
